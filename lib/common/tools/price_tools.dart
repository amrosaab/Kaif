...
class PriceTools {
  static Map<String, double> newRates = <String, double>{};

  static Map<String, double> getRatesFromEnv() {
    final List<Map<String, dynamic>> listOfCurrencies =
        environment['advanceConfig']['Currencies'];
    for (var element in listOfCurrencies) {
      newRates.addEntries(
        <String, double>{
          element['currencyCode']: double.parse('${element['rate']}')
        }.entries,
      );
    }
  ...
  static String? getCurrencyFormatted(price, Map<String, dynamic>? rates,
    ...
    try {
      ...
      if (rates[defaultCurrency.currencyCode] != null) {
